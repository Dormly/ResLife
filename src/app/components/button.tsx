...
import {  LucideIcon, ChevronDown, ChevronUp } from "lucide-react"; {/* TODO: Change to ChevronUp when dropdown active & button pressed */}
...
const Button: React.FC<ButtonProps> = ({ Icon, label, href = "", dropdown = false, imageSrc = "", bgColor="bg-white", hoverBgColor = "hover:bg-saffron", textColor = "text-black", hoverTextColor = "hover:text-white", width, height }) => {
    return (
        <Link href={href}>
            <div className={`${textColor} ${hoverTextColor} ${bgColor} ${hoverBgColor} ${width} ${height} min-h-[4rem] min-w-[4rem] flex flex-row items-center gap-[0.5rem] rounded-xl transition-all duration-100 p-[1.25rem] drop-shadow-[0px_4px_4px_rgba(0,0,0,0.25)] text-[1rem] font-bold`}>
                {Icon && <Icon size={24}/>}
